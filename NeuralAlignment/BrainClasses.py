import pandas as pd
from sklearn.linear_model import Ridge, LinearRegression, RidgeCV, Lasso, LassoCV, LassoLarsCV
...
class ANNEncoder:
    def __init__(self, source_model: str = 'gpt2',
        ...
        if source_model.startswith('gpt') or source_model.startswith('bert') or source_model.startswith('distil'):
            # Pretrained model
    ...
    def encode_from_csv(self,
        ...
        df_csv = pd.read_csv(join(CSVDIR,
                                  f'{CSV_fname}.csv'))
        # Index the dataframe by {stimsetid}.{item_id}
        df_csv.index = df_csv['item_id'].apply(lambda x: f'{stimsetid}.{x}')

        # Only get the indices that are indices in stimset (based on item_id)
        df_csv_encoded = df_csv[df_csv.item_id.isin(stimset.item_id.values)]
        ...
        # Find all cols that start with "layer" and end with the {sent_embed} (e.g. "layer12_last")
        layer_cols = [col for col in df_csv_encoded.columns if 'layer' in col and self.sent_embed in col]

        # Now put the activations in a df where the multiindex is the layer and the unit
        actv = pd.DataFrame()
        for layer_col in layer_cols: #e.g. layer9_last
            # Get the matrix out
            layer_matrix = df_csv_encoded[layer_col].apply(
                lambda x: np.fromstring(x.strip("[]"), sep=",") if isinstance(x, str) else np.array([])
            ).to_numpy()
            # Stack the rows into a proper 2D matrix
            actv_layer = np.vstack(layer_matrix)
            assert (actv_layer.shape[0] == df_csv_encoded.shape[0])

            # Get the layer idx, and add that as a multiindex
            ...
            actv_layer = pd.DataFrame(actv_layer, index=stimset.index)
            actv_layer.columns = pd.MultiIndex.from_product([[layer_idx], actv_layer.columns])

            actv = pd.concat([actv, actv_layer], axis=1)
    ...
    def encode_langloc_from_csv(self,
                       stimset: pd.DataFrame = None,
                       stim_col: str = 'sentence',
                       cache_new_actv: bool = True,
                       CSVDIR: str = None,
                       ...
                       **kwargs):
        """
        Load activations from a csv file which is the output of apply_langloc_mask.py.
        ...
        # Load the csv file
        df_csv = pd.read_csv(join(CSVDIR,
                                  f'{CSV_fname}.csv'), index_col=0, header=[0, 1]) # multi index csv
        # Convert MultiIndex levels to integers
        df_csv.columns = pd.MultiIndex.from_tuples([(int(l1), int(l2)) for l1, l2 in df_csv.columns])

        assert (stimset.shape[0] == df_csv.shape[0])

        # Index the dataframe by {stimsetid}.{item_id}
        df_csv.index = stimset['item_id'].apply(lambda x: f'{stimsetid}.{x}')
    ...
    def encode(self,
        ...
        for group in tqdm(context_groups[np.sort(unique_ixs)]):  # Make sure context group order is preserved
            ...
            for i, stimulus in enumerate(stim_in_context):
                ...
                # tokenized_directional_context.tokens()
...
class Metric:
    ...
    def _get_upper_triangular_indices(self, A):
        ...
        # A_masked = np.zeros_like(A)
        # A_masked[idx_upper] = np.asarray(A)[idx_upper]
        # plt.imshow(A_masked)
        # plt.colorbar()
        # plt.show()
...
class BrainEncoder:
    ...
    def encode(self,
        ...
        # Assert against drive/suppress data
        df = pd.read_csv('/Users/gt/Documents/GitHub/drive_suppress_brains/data/brain-lang-data_participant_20230728.csv')
        # query UIDs 848, 853, 865, 875, 876
        df = df[df['target_UID'].isin([848, 853, 865, 875, 876])]
        # roi = lang_LH_netw
        df_roi = df[df['roi'] == 'lang_LH_netw']
        # group by item_id
        df_roi_grouped = df_roi.groupby('item_id').mean()
        assert (np.allclose(df_roi_grouped.response_target.values, neural_data['lang_LH_netw'].values))
...
class Mapping:
    def __init__(self,
        ...
        mapping_classes = {
            ...
            'sparsepos01': (Lasso, {'alpha': 0.1, 'positive': True}),
            'sparsepos001': (Lasso, {'alpha': 0.01, 'positive': True}),
            'sparsepos0001': (Lasso, {'alpha': 0.001, 'positive': True}),
            'sparsepos00001': (Lasso, {'alpha': 0.0001, 'positive': True}),
            'sparsepos000001': (Lasso, {'alpha': 0.00001, 'positive': True}),
            'sparseposCV': (LassoCV, {'alphas': [0.00001, 0.0001, 0.001, 0.01, 0.1], 'positive': True, 'max_iter': 10000}),
            'sparseposCVLars': (LassoLarsCV, {'max_iter': 10000, 'positive': True}),
    ...
    def load_full_mapping_pretransformer(self, WEIGHTDIR,
                                         mapping_result_identifier: str = None,
                                         mapping_save_str: str = None,
                                         **kwargs):
        """Load the mapping classifier (weights, alphas (if any), and intercept)
        ...
        with open(join(WEIGHTDIR, 'fit_mapping_pretransformer', mapping_result_identifier, mapping_save_str),
                  'rb') as f:
            d = pickle.load(f)
    ...
    def RSA_score_neural(self,
                         permute_X: bool = False,
                         permute_X_seed: int = 0,
                         ):
        """
        Calculate RSA between the RSMs of the two neural matrices.
        We want to pass an X matrix (instead of taking the ANN activations).
        ...
        if permute_X:
            X = X.sample(frac=1).reset_index(drop=True,
                                             random_state=permute_X_seed)
            print(f'Permuting X before calculating RSA using random seed {permute_X_seed}')
        ...
        # Then, use metric
        score = self.metric._metric_over_neuroids(A=X_RSM_upper, B=y_RSM_upper)
...
class ArgumentLogger:
    ...
    def create_result_identifier(self):
        ...
        if self.script_name == 'fit_mapping':
        ...
        elif self.script_name == 'fit_mapping_for_ceiling':
            self.result_identifier = f'SOURCE-{self.args["source_regression_dict_type"]}_' \
                                     f'{self.args["source_UID"]}_' \
                                     f'{self.args["source_sess_num"]}_' \
                                     f'{self.args["source_FL"]}_' \
                                     f'{self.args["source_func_thresh"]}_' \
                                     f'TARGET-{self.args["regression_dict_type"]}-' \
                                     f'{self.args["UID"]}_' \
                                     f'{self.args["sess_num"]}_' \
                                     f'{self.args["FL"]}_' \
        ...
        elif self.script_name == 'check_ANNEncoder':
            self.result_identifier = f'SOURCE-{self.args["source_model"]}_' \
    ...
    def create_save_str(self):
        ...
        if self.script_name == 'fit_mapping' or self.script_name == 'fit_mapping_pretransformer':
            ...
            self.save_str = f'SOURCE-{self.args["source_savestr_prefix"]}-' \
                            f'{self.args["source_layer"]}_' \
                            f'{self.args["source_modeltype"]}-' \
                            f'{self.args["source_preproc"]}-' \
                            f'{self.args["source_pcstop"]}-' \
                            f'{self.args["source_fracs"]}-' \
                            f'{self.args["source_norm"]}_' \
                            f'TARGET-{self.args["savestr_prefix"]}-' \
                            f'{self.args["specific_target"]}_' \
                            f'{self.args["modeltype"]}-' \
                            f'{self.args["preproc"]}-' \
            ...
            self.save_str = f'{self.args["general_corpus"]}-' \
                            f'{self.args["corpus"]}-' \
                            f'{self.args["part_count"]}'
